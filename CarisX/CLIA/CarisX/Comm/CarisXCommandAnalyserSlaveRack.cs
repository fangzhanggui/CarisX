...
namespace Oelco.CarisX.Comm
{
    ...
    /// </remarks>
    public class CarisXCommandAnalyserSlaveRack : ICommCommandAnalyser
    {
        #region [定数定義]
        ...
        static protected Type commandIdentify( String commandIdIn )
        {
            ...
            int commandKind = (int)CommandKind.Unknown;

            try
            {
                commandKind = int.Parse(commandId);
            }
            catch(Exception ex)
            {
                Console.WriteLine("StackTrace={0}, Message={1}",ex.StackTrace, ex.Message);
                Console.WriteLine("commandIdIn={0}", commandIdIn);
            }

            switch (commandKind)
            {
                #region [RackTransferCommCommand]
