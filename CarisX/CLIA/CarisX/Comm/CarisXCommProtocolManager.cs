...
namespace Oelco.Common.Comm
{
    ...
    public delegate void CarisXCommSendResult( Int32 retCode, CommCommand cmd );
    ...
    /// </remarks>
    public class CarisXCommProtocolManager
    {
        #region [インスタンス変数定義]
        ...
        /// 通信インターフェース
        /// </summary>
        private CarisXCommSocket commObject = null;
        ...
        /// コマンド送信結果変数
        /// </summary>
        protected CarisXCommSendResult dlgCommSendResult;
        ...
        /// 通信インターフェース 取得/設定
        /// </summary>
        public CarisXCommSocket CommObject
        {
            get
            {
        ...
        /// コンストラクタ
        /// </summary>
        public CarisXCommProtocolManager()
        {
            this.initialize();
        }
        ...
        /// </summary>
        /// <remarks>
        /// CommSocketのClose()の前に呼ぶ対スレーブ用切断処理
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// 接続を行い、対スレーブ用通信ログの設定を行う
        /// </remarks>
        /// <param name="parameter">接続パラメータ</param>
        /// <param name="saveFilePath">通信ログファイルパス</param>
        /// <returns>True:成功 False:失敗</returns>
        public Boolean Connect(List<SocketParameter> parameter, List<String> saveFilePath)
        {
            Boolean openSuccess = false;
            ...
            if (openSuccess == true)
            {
                ...
                for( int idx = 0; idx < saveFilePath.Count;  idx++ )
                {
                    ...
                    // 通信ログファイルパス設定
                    this.CommObject.SaveFilePath(saveFilePath[idx], (short)parameter[idx].ModuleId);
                }
            }
        ...
        public virtual void SetCommSendResultCallBack( CarisXCommSendResult commSendResult )
        {
        ...
        public virtual Boolean Send( CommCommand command)
        {
            ...
            if ( this.commObject != null )
            {
                ...
                {
                    // コマンドID取得
                    ret = this.commObject.SendText( command.CommandId.ToString( "d4" ) + command.CommandText, (short)command.CommNo );
                }
                catch ( Exception ex )
                {
                    Singleton<LogManager>.Instance.WriteCommonLog( LogKind.DebugLog, String.Format( "*Command Send Error![{3}] Type = {0} Message = {1} StackTrace = {2}", command == null ? "unknown" : command.GetType().Name, ex.Message, ex.StackTrace, command.CommNo) );
                ...
                Singleton<LogManager>.Instance.WriteCommonLog( LogKind.DebugLog, String.Format( "*{3}Command Sended[{4}] : Type = {0} success = {1} detail = {2}", command == null ? "unknown" : command.GetType().Name, sendSuccess.ToString(), ret.ToString(), DateTime.Now.ToString( "yyyy/MM/dd HH:mm:ss" ), command.CommNo) );
                System.Diagnostics.Debug.WriteLine( String.Format( "*{3}Command Sended[{4}] : Type = {0} success = {1} detail = {2}", command == null ? "unknown" : command.GetType().Name, sendSuccess.ToString(), ret.ToString(), DateTime.Now.ToString( "yyyy/MM/dd HH:mm:ss" ), command.CommNo) );
        ...
        public virtual Boolean Receive( out CommCommand command )
        {
            ...
            if ( this.commObject != null )
            {
                String strRecvBuff;
                short machineCode = 0;
                Int32 ret = this.commObject.RecvText( out strRecvBuff, out machineCode );
                if ( ret > 0 )
                {
                    recvSuccess = true;
                }
                ...
                if ( recvSuccess )
                {
                    ...
                    if ( command == null )
                    {
                        ...
                        Singleton<LogManager>.Instance.WriteCommonLog( LogKind.DebugLog, String.Format( "*{1}Command analyse failed[{2}] : String = {0}", strRecvBuff, DateTime.Now.ToString( "yyyy/MM/dd HH:mm:ss" ), machineCode) );
                        recvSuccess = false;
                    }
                    else
                    {
                        Singleton<LogManager>.Instance.WriteCommonLog( LogKind.DebugLog, String.Format( "*{1}Command Received[{3}] : Type = {0} Text = {2}", command == null ? "unknown" : command.GetType().Name, DateTime.Now.ToString( "yyyy/MM/dd HH:mm:ss" ), strRecvBuff, machineCode) );
                        System.Diagnostics.Debug.WriteLine( String.Format( "*{1}Command Received[{3}] : Type = {0} Text = {2}", command == null ? "unknown" : command.GetType().Name, DateTime.Now.ToString( "yyyy/MM/dd HH:mm:ss" ), strRecvBuff, machineCode) );
        ...
        /// 接続状態の取得
        /// </summary>
        public ConnectionStatus GetCommStatus(short id)
        {
            ConnectionStatus result = ConnectionStatus.Error;
            if (this.commObject != null)
            {
                // インターフェース経由でパラメータ設定
                result = this.commObject.GetConnectionStatus(id);
            }
            return result;
        }
        ///// <summary>
        ...
        {
            this.CommObject = new CarisXCommSocket();
        }
        ...
        protected void commandReflection( CommCommand cmd )
        {
            ...
            {
                Int32 NEST_LIMIT = 5;
                System.Diagnostics.Debug.WriteLine( String.Format("-----[{0}]-------------------↓commandDetail↓---------------------------", cmd.CommNo) );
