...
namespace Oelco.CarisX.DB
{
    ...
    /// 再測定STAT検体グリッド表示内容
    /// </summary>
    public class SpecimenStatReMeasureGridViewDataSet
    {
        /// <summary>
        /// 分析モジュール番号
        /// </summary>
        ...
        /// 測定指示問い合わせ待機フラグ
        /// </summary>
        public SpecimenCupKind VesselType = SpecimenCupKind.None;
    }
    ...
    /// </summary>
    /// <remarks>
    /// 再測定STAT検体DBアクセスクラスです。
    /// </remarks>
    class SpecimenStatReMeasureDB : DBAccessControl
    {
        /// <summary>
        ...
        protected override String baseTableSelectSql
        {
            get
            {
                return "SELECT * FROM dbo.specimenStatReMeasure";
            }
        }
        ...
        /// <param name="data">測定指示データMeasurement instruction data</param>
        /// <param name="sequenceNo">sequence番号</param>
        /// <param name="receiptNumber">受付番号</param>
        /// <param name="comment">コメント</param>
        /// <returns>True:取得成功　False:取得失敗</returns>
        public Boolean MeasureIndicate(AskTypeKind askType, int moduleId, ref IMeasureIndicate data, out Int32 sequenceNo, out Int32 receiptNumber, out String comment)
        {
            ...
            CarisXIDString rackId = data.RackID;        //E000のみ
            Int32 rackPos = moduleId;                   //モジュールID依存
            ...
            {
                // データリスト生成
                IEnumerable<SpecimenStatReMeasureGridViewDataSet> dataListTmp = from v in result
                ...
                if (first.WaitMeasureIndicate)
                {
                    ...
                    data.SpecimenCup = first.VesselType;
        ...
        public List<SpecimenStatReMeasureGridViewDataSet> GetDispData(GetAutoRemeasKind getAutoReMeasureData = GetAutoRemeasKind.Disp)
        {
            List<SpecimenStatReMeasureGridViewDataSet> result = new List<SpecimenStatReMeasureGridViewDataSet>();
            ...
            if (this.DataTable != null)
            {
                try
                {
                    ...
                    {
                        SpecimenStatReMeasureGridViewDataSet data = getDataStruct(row, getAutoReMeasureData);
                        if (data != null)
                        {
                            result.Add(data);
                        }
        ...
        private SpecimenStatReMeasureGridViewDataSet getDataStruct(System.Data.DataRow row, GetAutoRemeasKind targetData)
        {
             SpecimenStatReMeasureGridViewDataSet data = null;
            ...
            if (visibleData)
            {
                data = new SpecimenStatReMeasureGridViewDataSet();
                ...
                data.VesselType = row.Field<SpecimenCupKind>(STRING_VESSELTYPE);
            }
        ...
        /// <returns>True:成功 False:失敗</returns>
        public Boolean SetDispData(List<SpecimenStatReMeasureGridViewDataSet> dataList)
        {
            Boolean result = true;
            if (this.DataTable != null)
            {
                try
                {
                    ...
                    // テーブル内容に更新する
                    foreach (SpecimenStatReMeasureGridViewDataSet data in dataList)
                    {
        ...
        /// <returns>True:成功 false:失敗</returns>
        public Boolean AddRemesureData(List<SpecimenReMeasureGridViewDataSet> dataList, SpecimenCupKind vesselType)
        {
            Boolean result = true;
            if (this.DataTable != null)
            {
                try
                {
                    ...
                    if (result)
                    {
                        ...
                        foreach (var data in datas)
                        {
                            ...
                            newRow.SetField<Int32>(STRING_VESSELTYPE, (Int32)vesselType);
