    ...
    public class CalibratorAssayDB : DBAccessControl
    {
        ...
        public List<CalibratorAssayData> GetData(CarisXIDString rackID = null)
        {
            ...
            if (this.DataTable != null)
            {
                try
                {
                    var datas = from data in this.DataTable.Copy().AsEnumerable().AsParallel().Select((row) => new CalibratorAssayData(row))
        ...
        public Boolean AssayStatusUpdate(Int32 uniqueNo, Int32 replicationNo, TimeSpan remain)
        {
            ...
            var changeData = this.DataTable.AsEnumerable().AsParallel().Select((row) => new CalibratorAssayData(row)).FirstOrDefault((targetData) => targetData.GetUniqueNo() == uniqueNo && targetData.ReplicationNo == replicationNo);
            if (changeData != null)
            {
                // ステータス更新
